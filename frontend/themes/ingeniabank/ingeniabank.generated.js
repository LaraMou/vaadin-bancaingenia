...
window.Vaadin.Flow['_vaadintheme_ingeniabank_globalCss'] = window.Vaadin.Flow['_vaadintheme_ingeniabank_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_ingeniabank_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_ingeniabank_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_ingeniabank_globalCss'].push(target);
  }
  if (!document['_vaadintheme_ingeniabank_componentCss']) {
    registerStyles(
      'vaadin-button',
      css`
    ...
    document['_vaadintheme_ingeniabank_componentCss'] = true;
  }
